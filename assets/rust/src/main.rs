...
#[wasm_bindgen(module = "/js/helper.js")]
extern "C" {
    fn postParent(message: &str);
...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(js_namespace = ["window", "parent", "CHANNEL"], js_name = "push")]
    fn phoenix_push(name: &str, payload: &JsValue);

    #[wasm_bindgen(js_namespace = ["window", "parent", "CHANNEL"], js_name = "on")]
    fn phoenix_on(name: &str, callback: &Function);
}
...
fn main() {
    ...
    let closure = Closure::wrap(Box::new(move |payload: JsValue| {
        ...
        let example: Example = serde_wasm_bindgen::from_value(payload).expect("malformde payload");
    ...
    phoenix_on("shout", closure.as_ref().unchecked_ref());

    phoenix_push(
        "shout",
        &serde_wasm_bindgen::to_value(&Example {
            field1: [0.0, 0.0, 0.0, 0.0],
        })
        .expect("is right type"),
    );
    ...
    //     let example: Example =
    //         serde_wasm_bindgen::from_value(event.data()).expect("malformde event data");
...
fn listen_from_javascript(
    receiver: Res<ReceiverResource<Example>>,
